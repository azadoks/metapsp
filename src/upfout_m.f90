...
! for PWSCF input using the UPF file format
 ...
 subroutine upfout_m(lmax,lloc,rc,vkb,evkb,nproj,rr,vpuns,rho,rhomod, &
&                  taups,taumod, &
...
!taups valence pseudo-kinetic energy density
...
&      '    <PP_HEADER'
   ...
   write(6,'(t8,a)') &
&        'generated="Generated using METAPSP code by D. R. Hamann"'
   write(6,'(t8,a)') &
&        'author="anonymous"'
   write(6,'(t8,5a)') &
&        'date="',pspd(:),'"'
...
&      '<PP_NONLOCAL>'
 ...
 do l1=1,lmax+1
   ...
   do jj=1,nproj(l1)
     ...
     if(iproj<=9) then
        write(cproj,'(i1,1x)') iproj
     else
        write(cproj,'(i2)') iproj
     end if
     write(6,'(t4,a)') &
&            '<PP_BETA.'//trim(cproj)
     write(6,'(t8,a)') &
...
! Additions for metagga -- valence tau and model "core" tau
