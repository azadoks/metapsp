...
!
 subroutine run_diag_m(lmax,npa,epa,lloc,irc,vkb,evkb,nproj,rr, &
&                      vfull,vtau,vp,vtaups,zz,mmax,mxprj,srel,ircmin)
...
!Input variables
 integer :: lmax,lloc,mmax,mxprj,nlim,ircmin
...
!Local variables
 ...
 real(dp) :: al,emax,emin,ebar,etest,sls,umch,upmch,uldf,gam,gpr
...
&                rr,vfull,vtau,uu,up,zz,mmax,mch,srel,ircmin)
      if(ierr .ne. 0) then
         write(6,'(/a,3i4)') 'run_diag_m: lschfb_m convergence ERROR n,l,ierr=', &
&        npa(iprj,l1),ll,ierr
!       stop
      end if

    else
      call lschfs_m(nnae,ll,ierr,etest, &
&               rr,vfull,vtau,uu,up,zz,mmax,mchf,srel,ircmin)
    end if
...
&                   uu,up,mmax,mch)
      if(ierr/=0) then
        write(6,'(a,3i4,1p,2e16.8)') 'run_diag_m: lschvkbb_m ERROR', &
...
!     from all-electron node count
      ...
      call lschvkbbe_m(nnp,ll,npr,ierr,etest,uldf,emin,emax, &
&                   rr,vp(1,lloc+1),vtaups,vkb(1,1,l1),evkb(1,l1), &
&                   uu,up,mmax,mchf)

      if(ierr/=0) then
        write(6,'(a,4i4,1p,2e16.8)') 'run_diag_m: lschvkbbe_m ERROR', &
