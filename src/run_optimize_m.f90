...
!
   subroutine run_optimize_m(eig,ll,mmax,mxprj,rr,uua,qq,&
&                        irc,qcut,qmsbf,ncon_in,nbas_in,npr, &
&                        psopt,pspopt,psppopt,vpsp,vkb,vae,cvgplt,nderr)
...
!Output variables
 real(dp) :: qmsbf
 real(dp) :: psopt(mmax,mxprj),pspopt(mmax,mxprj),psppopt(mmax,mxprj)
 real(dp) :: vpsp(mmax),vkb(mmax,mxprj)
 logical :: nderr
...
!Local variables
 ...
 real(dp) :: sbf(mmax)
 real(dp) :: cons(10),cvgplt(2,7,mxprj)
...
!  qmax=300.0d0  !very large value should always allow nbas q's to be found
   qmax=500.0d0  !very large value should always allow nbas q's to be found
...
! find the Vanderbilt projectors and optimized wave functions
   ...
   call pspot_m(iprj,ll,rr,irc,mmax,al,nbas,qroot,eig(iprj),uua(1,iprj), &
&             pswfopt_sb,psopt(1,iprj),pspopt(1,iprj),psppopt(1,iprj), &
&             vae,work,vkb(1,iprj),ekin_num,nderr)
 if(nderr) then
   return
 end if

!semi-local potential - NOT VALID for any use except qualitative plots
   if(iprj==1) then
     do ii=1,irc
       vpsp(ii)=work(ii)
     end do
