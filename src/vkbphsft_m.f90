! Copyright (c) 1989-2024 by D. R. Hamann, Mat24im Research LLC and Rutgers
! University
!
! 
...
!
 subroutine vkbphsft_m(ll,ivkb,epsh2,depsh,ep,pshf,pshp, &
&                      rr,vloc,vtau,vkb,evkb,mmax,mch,npsh)
...
!Input variables
 ...
 real(dp) :: rr(mmax),vloc(mmax),vkb(mmax,*),vtau(mmax),evkb(*)
 real(dp) :: pshf(npsh)
...
!Local variables
 ...
 real(dp), allocatable :: uu(:),up(:),dvtau(:)

 allocate(uu(mmax),up(mmax),dvtau(mmax))
...
&          -2.d0*vtau(ii+2))/(24.d0*al*rr(ii))
 ...
 do ii = 1,npsh
   ...
   call vkboutwf_m(ll,ivkb,epsh,vkb,evkb,rr,vloc,vtau,dvtau, &
&                  uu,up,node,mmax,mch)
...
!  end if
 ...
 deallocate(uu,up,dvtau)
 return
 end subroutine vkbphsft_m
