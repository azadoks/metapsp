...
!
 subroutine lschvkbs_m(ll,nvkb,ee,rr,vloc,vtau,vkb,evkb,uu,up,mmax,mch)

! Normalized scattering state for pseudopotential, fully non-local unless 
! nvkb=0, which should only happen if ll=lloc

!ll  angular-momentum quantum number
!nvkb  = 0, 1 or 2 VKB proectors to be used
...
!Input variables
 integer :: mmax,mch
 real(dp) :: rr(mmax),vloc(mmax),vtau(mmax),vkb(mmax,2),evkb(2)
 real(dp) :: ee
 integer :: nvkb,ll
