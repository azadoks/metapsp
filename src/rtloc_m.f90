...
!
 subroutine rtloc_m(rr,vv,vp,dvloc0,irc,mmax,lpopt)

!polynomial extrapolation of all-electron potential to rr=0
!repurposed to extend AE of AE-PS rho of tau to origin.  ONly change
!is to replace historic vp(:,5) by vp(:)
...
! create polynomial potential inside irc
 ...
 if(lpopt .eq. 1) then
   do ii=1,irc
     vp(ii)=aco+bco*rr(ii)**2+cco*rr(ii)**4
   end do
 else if(lpopt .eq. 2) then
   do ii=1,irc
     vp(ii)=aco+bco*rr(ii)**4+cco*rr(ii)**6
   end do
 else if(lpopt .eq. 3) then
   do ii=1,irc
     vp(ii)=aco+bco*rr(ii)**4+cco*rr(ii)**5+dco*rr(ii)**6
   end do
 else if(lpopt .eq. 4) then
   do ii=1,irc
     vp(ii)=aco+bco*rr(ii)**4+cco*rr(ii)**6+dco*rr(ii)**8
   end do
 else if(lpopt .eq. 5) then
   do ii=1,irc
     vp(ii)=aco+bco*rr(ii)**2+cco*rr(ii)**4+dco*rr(ii)**6
   end do
 end if
...
! add compatible supplementary function to change value at rr==0
 ...
 do ii = 1, irc
   x=rr(ii)/rr(irc)
   if(lpopt .eq. 1) then
     vp(ii)=vp(ii) + dvloc0*(1.0d0 - x**2)**3
   else if(lpopt .eq. 2) then
     vp(ii)=vp(ii) + dvloc0*(1.0d0 - x**4)**3
   else if(lpopt .eq. 3 .or. lpopt .eq. 4) then
     vp(ii)=vp(ii) + dvloc0*(1.0d0 - x**4)**4
   else if(lpopt .eq. 5) then
     vp(ii)=vp(ii) + dvloc0*(1.0d0 - x**2)**4
   end if
 end do
 ...
 return
 end subroutine rtloc_m

!NOTES: (1-x)**4 = 1 - 4*x + 6*x**2 - 4*x**3 + x**4
