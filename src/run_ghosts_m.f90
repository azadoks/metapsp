...
!
 subroutine run_ghosts_m(lmax,la,ea,nc,nv,lloc,irc,qmsbf, &
&                    vkb,evkb,nproj,rr,vp,vtau,mmax,mxprj)
...
!Input variables
 ...
 real(dp) :: rr(mmax),vp(mmax,5),vtau(mmax),vkb(mmax,mxprj,4)
...
!Local variables
 ...
 real(dp) :: al,amesh,basfact,ee,ecut,eps,et,emin,emax,ebar,sn,ro,tht
...
! with 20% allowance for binding by local potential
   ...
   do jj=1,nbmax
     ...
     call lschpsbar_m(nn,ll,ierr,ee,emin,emax,rr,vp(1,lloc+1),vtau, &
&                     uu,up,mmax,mbar,tht)

     if(ierr/=0) then
         if(emax/=ebcut(l1)) then
         write(6,'(a,3i4,3f12.6)')  &
&              'run_ghosts 150: lschpsbar_m ERROR ierr,nn,ll,ee,emin,emax', &
...
! overlap of eigenfunction jj and projectors
     do iprj=1,nprj
       call vpinteg(uu,vkb(1,iprj,l1),irc(l1),2*ll+2,gg0(iprj,jj),rr,mmax)
     end do
...
!treat valence states for this l
     ...
     do kk=1,nv
       ...
       call lschvkbb_m(nn,ll,nprj,ierr,ee,emin,emax,ebar, &
&                     rr,vp(1,lloc+1),vtau,vkb(1,1,l1),evkb(1,l1), &
&                     uu,up,mmax,mch)

       if(ierr/=0) then
         write(6,'(a,3i4,2f12.6)') 'run_ghosts: lschvkbb_m ERROR', &
...
!if this fails, use vacuum level (0.0) to compare to basis-set states
     ...
     if(jj==0) then
       ...
       emax=0.0d0
       emin=2.0d0*ee
       call lschvkbb_m(ll+1,ll,nprj,ierr,ee,emin,emax,ebar, &
&                    rr,vp(1,lloc+1),vtau,vkb(1,1,l1),evkb(1,l1), &
&                    uu,up,mmax,mch)
       if(ierr/=0) ee=0.0d0
       do jj=1,10
...
&                +  9.0d0*(rr(mbar  )*uu(mbar  ))**2)/24.0d0
   ...
   write(6,'(/a)') ' Testing for highly-localized ghosts above bound states'
   write(6,'(5a)') '    ','   l','    <radius>/rc  ', &
&        '   E Basis Diag.','   E Cutoff'

 if(npgh>0) then
   do jj=1,npgh
        if(epgh(jj)>0.0d0) then
