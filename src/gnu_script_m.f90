...
! creates appropriate input file for GNUPLOT
 ...
 subroutine gnu_script_m(epa,evkb,lmax,lloc,mxprj,nproj)
...
!Local variables
 ...
 character(len=50) :: plot_title(12)
 ...
 plot_title(1)='set title "t2   Unscreened Semi-Local Guides"'
 ...
 plot_title(11)='set title "t2   Kinetic Energy Densities"'
 plot_title(12)='set title "t2   Vtaus (Ha / a_B)"'
 ...
 gln( 6)='    "<grep ''!r'' t1" using 2:3 title "rhoPS" with lines ls 1'
...
!metagga additions
 ...
 gln(76)='    "<grep ''!t'' t1" using 2:3 title "tauPS" with lines ls 1'
 gln(77)='    "<grep ''!t'' t1" using 2:4 title "tau(M+PS)" with lines ls 9'

 gln(78)='    "<grep ''!vt'' t1" using 2:3 title "VtauAE" with lines ls 1'
 gln(79)='    "<grep ''!vt'' t1" using 2:4 title "Vtau(M+PS)" with lines ls 9'

!write(6,*) 'lmax,lloc,mxprj',lmax,lloc,mxprj
 write(6,'(a)') 'GNUSCRIPT_m'
...
!plot taus 
 ...
 do ii=1,2
  write(6,'(2a)',advance='no') lead(ii),trim(gln(ii+75))
  if(ii<2) then
   write(6,'(a)') cbs
  else
   write(6,'(/)')
 ...
 set='unset  logscale y'
 write(6,'(a/)') trim(set)

 write(6,'(/a/)') trim(pse)

!metagga vtau potentials

!plot taus 
 write(6,'(a/)') trim(plot_title(12))
 write(6,'(a/)') trim(x_label(1))
 ...
 do ii=1,2
  write(6,'(2a)',advance='no') lead(ii),trim(gln(ii+77))
  if(ii<2) then
   write(6,'(a)') cbs
  else
   write(6,'(/)')
