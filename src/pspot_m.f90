...
!
 subroutine pspot_m(ipr,ll,rr,irc,mmax,al,nbas,qroot,eig,uu,pswfopt_sb, &
&           psopt,pspopt,psppopt,vae,vpsp,vkb,ekin_num,nderr)
...
!nderr .true. for node error, .false. otherwise
...
!Output variables
 real(dp) :: psopt(mmax),pspopt(mmax),psppopt(mmax)
...
!Local variables
 ...
 real(dp) :: sbfder(5)
 real(dp) :: amesh,tt,dt,ddt,ske,ro,sls
...
!fill [0, rc] with rr * analytic pswf and 2nd derivatives
 do ii=1,irc
  ...
  do jj=1,nbas
   ...
   dt=dt+pswfopt_sb(jj)*(rr(ii)*sbfder(2)+sbfder(1))
 ...
 do ii=1,mmax
  ...
  pspopt(ii)=al*rr(ii)*work(ii,2)

  psppopt(ii)=(al*rr(ii))**2*work(ii,3)+(al**2)*rr(ii)*work(ii,2)
 end do
...
!alternative form for testing
        sls=ll*ll1
!       work(:,5)=0.5d0*(work(:,2)**2 + sls*(work(:,1)/rr(:))**2)



 ro=rr(5)/sqrt(amesh)
!ske=((work(5,5))/rr(5)**ixp(ll1)) * ro**ixp(ll1+1)/ixp(ll1+1)
 ske=ro*work(5,5)*sqrt(amesh)**ixp(ll1)
