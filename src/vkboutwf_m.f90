...
!
 subroutine vkboutwf_m(ll,nvkb,ep,vkb,evkb,rr,vloc,vtau,dvtau, &
&                    uu,up,node,mmax,mch)
...
!Input variables
 real(dp) :: rr(mmax),vloc(mmax),vtau(mmax),dvtau(mmax)
 real(dp) :: vkb(mmax,nvkb),evkb(nvkb)
 real(dp) :: ep
...
real(dp) :: rc,rn
 ...
 if(mch<=1) then
   write(6,'(/a,i6)') 'vkboutwf_m called with mch <',mch ; flush 6
   stop
 end if
 uu(:)=0.0d0
 up(:)=0.0d0
! homogeneous solution
 call lschps_m(ll+1,ll,ierr,ep,rr,vloc,vtau,dvtau,uu,up,mmax,mch)
...
! inhomogeneous solutions
  do ii=1,nvkb
   call lschkb_m(ll+1,ll,ierr,ep,vkb(1,ii),rr,vloc,vtau,dvtau, &
&                phi(1,ii),phip(1,ii),mmax,mch)
  end do
...
! projector matrix elements and coefficient matrix
  do jj=1,nvkb
   ...
   do ii=1,nvkb
    call vpinteg(phi(1,ii),vkb(1,jj),mch,2*ll+2,gg(jj,ii),rr,mmax)
